...
export default function Footer() {
  ...
  return (
    <footer className="relative bg-onyx-950 text-white overflow-hidden">
      {/* Decorative Elements */}
      <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-gold-500/50 to-transparent" />
      <div className="absolute top-0 right-0 w-96 h-96 bg-gold-500/5 rounded-full blur-3xl -translate-y-1/2" />
      <div className="absolute bottom-0 left-0 w-72 h-72 bg-onyx-600/10 rounded-full blur-3xl translate-y-1/2" />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 mb-16">
          ...
          <div className="lg:col-span-1">
            ...
            <div className="flex gap-3">
              {[Instagram, Facebook, Linkedin].map((Icon, i) => (
                ...
                >
                  <Icon className="w-4 h-4 text-white/50 group-hover:text-onyx-900 transition-colors" />
                </a>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <Link
              href={`/${locale}/contact`}
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-gold-500 to-gold-400 text-onyx-900 text-sm font-semibold rounded-xl hover:from-gold-400 hover:to-gold-300 transition-all duration-300 shadow-lg shadow-gold-500/20 hover:shadow-gold-500/40 hover:-translate-y-0.5"
            >
              {t('nav.requestQuote')}
        ...
        <div className="border-t border-white/5 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6">
            <a
              href={locale === 'tr' ? '/gizlilik-politikasi.html' : '/privacy-policy.html'}
              target="_blank"
            ...
            <a
              href={locale === 'tr' ? '/kullanim-sartlari.html' : '/terms-of-use.html'}
              target="_blank"
              rel="noopener noreferrer"
              className="text-white/30 hover:text-white/60 text-sm transition-colors"
            >
